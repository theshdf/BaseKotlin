...
import `fun`.shdf.basekotlin.AppApplication
import `fun`.shdf.basekotlin.util.NetUtil
    ...
    init {
        if (okHttpClient == null) {
            okHttpClient = OkHttpClient.Builder().addInterceptor(Interceptor {
                //缓存文件夹
                val cacheFile = File(AppApplication.getApplicationContext().getExternalCacheDir().toString(), "cache")
                //缓存大小为10M
